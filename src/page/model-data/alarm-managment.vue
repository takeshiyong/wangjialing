        ...
        <div class="page_container  ">
            <el-form :inline="true" :model="pageParam" class="demo-form-inline">
                <el-form-item
                    ...
                    prop="fAlarmNo">
                    <el-input
                        v-model="pageParam.fAlarmNo"
                        autocomplete="off"
                        placeholder="请输入报警点编号"
                        clearable
                ...
                <el-form-item label="处理状态" label-width="100px">
                    <el-select filterable v-model="pageParam.fState" placeholder="请选择处理状态" clearable style="width: 200px;">
            ...
            <div class="tab_container">
                <el-table ref="table" :data="tabList" stripe class="defaultTab"  :max-height="docHeight-350">
                    ...
                    <el-table-column prop="fAlarmNo" label="报警点编号" min-width="130" :show-overflow-tooltip="true">
                    </el-table-column>
                    ...
                    <el-table-column prop="fUnit" label="单位" min-width="130" :show-overflow-tooltip="true">
                        <template slot-scope="scope">
                    ...
                    <el-table-column prop="fModelDataName" label="模型数据参数名称" min-width="100" :show-overflow-tooltip="true">
                        <template slot-scope="scope">
                            <span>{{ scope.row.fModelDataName==null?'--':scope.row.fModelDataName }}</span>
                        </template>
                    </el-table-column>
                    <el-table-column prop="fModelDataNo" label="模型数据参数编号" min-width="100" :show-overflow-tooltip="true">
                        <template slot-scope="scope">
                            <span>{{ scope.row.fModelDataNo==null?'--':scope.row.fModelDataNo }}</span>
                        </template>
                    </el-table-column>
                    <el-table-column prop="fPushTypeName" label="推送类型名称" min-width="100" :show-overflow-tooltip="true">
                        <template slot-scope="scope">
                            <span>{{ scope.row.fPushTypeName==null?'--':scope.row.fPushTypeName }}</span>
                        </template>
                    </el-table-column>
            ...
            <TheDialog :modal='showCurrentRow' :modalTitle="modalTitle" @closeModal="showCurrentRow=false">
                <div class="box" slot="custom-modal-content">
                    <div class="center-box">
                        ...
                        <div class="view-details-rows">
                            <div class="view-details-row-item">
                                <span class="field-fname">模型数据参数名称</span>
                                <span class="field-content">{{currentRow.fModelDataName?currentRow.fModelDataName: '--'}}</span>
                            </div>
                            <div class="view-details-row-item">
                                <span class="field-fname">模型数据参数编号</span>
                                <span class="field-content">{{currentRow.fModelDataNo?currentRow.fModelDataNo: '--'}}</span>
                            </div>
                        </div>

                        <div class="view-details-rows">
                            <div class="view-details-row-item">
                                <span class="field-fname">推送类型名称</span>
                                <span class="field-content">{{currentRow.fPushTypeName?currentRow.fPushTypeName: '--'}}</span>
                            </div>

                            <div class="view-details-row-item">
                                <div class="field-content" style="width: 100%"></div>
                            </div>
                        </div>
...
import { ModelAlarmService } from '@/api/model-data.js';
        ...
        methods: {
            ...
            deleteAlarm(row) {
                this.$confirm(`您将删除这报警信息, 是否继续?`, '提示', {
                    ...
                    const res = await ModelAlarmService.deleteAlarm(params);
                    if (res.success) {
            ...
            async oneClearAlarmAjax() {
                    const res = await ModelAlarmService.dispostAllAlarm();
                    if (res.success) {
                        this.pageParam.currentPage = 1;
                        this.searchList();
                        this.$message({
            ...
            async elimination() {
                    ...
                    const res = await ModelAlarmService.disposeAlarm({
                        fCount: this.fCount,
            ...
            async searchList() {
                ...
                const res = await ModelAlarmService.getModelAlarms(param);
                if (res.success) {
